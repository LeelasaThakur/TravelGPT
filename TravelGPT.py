...
def generate_itinerary(destination_city):
    ...
    """
    prompt = f"""
    Create a 5-day travel itinerary for {destination_city} in a structured, clear, and visually appealing format. Each day should be broken down as follows:
...
def fetch_bookings_by_name(name):
    """
    Fetches all flight and hotel bookings associated with the given name.
    """
    ...
    if os.path.exists(PASSENGER_FILE):
        with open(PASSENGER_FILE, "r") as file:
            ...
            bookings["flights"] = [p for p in passengers if p.get("name") == name]

    # Fetch hotel bookings
    if os.path.exists(HOTEL_BOOKINGS_FILE):
        with open(HOTEL_BOOKINGS_FILE, "r") as file:
            hotels = json.load(file)
            bookings["hotels"] = [h for h in hotels if h.get("name") == name]

    if not bookings["flights"] and not bookings["hotels"]:
        return {"error": "No bookings found for this name."}
...
def fetch_bookings():
    """
    API endpoint to get bookings by user name.
    """
    data = request.json
    name = data.get("name")
    ...
    bookings = fetch_bookings_by_name(name)
            ...
            <div class="flex-1 flex flex-col glass rounded-2xl overflow-hidden">
                ...
                <div class="flex gap-4 p-4 overflow-x-auto">
                    <button class="quick-action flex-none px-4 py-2 bg-blue-100 text-blue-800 rounded-lg hover:bg-blue-200">
                        <i class="fas fa-plane-departure mr-2"></i>Flight Search
                    </button>
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            function initializeDemoContent() {
                ...
                document.getElementById("tripSummary").innerHTML = `
                    <div class="space-y-2">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="font-semibold">Paris, France</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-gray-600">Dates</span>
                            <span class="font-semibold">Jun 15 - Jun 22</span>
                        </div>
                        <div class="flex items-center justify-between">
                ...
                document.getElementById("upcomingTrips").innerHTML = `
                    <div class="sidebar-card p-3 bg-blue-50 rounded-xl">
                        <div class="font-semibold">Paris Adventure</div>
                        <div class="text-sm text-gray-600">June 15 - 22, 2024</div>
                        <div class="mt-2 text-xs text-blue-600">View Details →</div>
                    </div>
                    <div class="sidebar-card p-3 bg-green-50 rounded-xl">
                        <div class="font-semibold">Tokyo Expedition</div>
                        <div class="text-sm text-gray-600">August 3 - 10, 2024</div>
                        <div class="mt-2 text-xs text-green-600">View Details →</div>
                    </div>
                `;
            ...
            async function sendMessage() {
                ...
                setTimeout(() => {
                    ...
                    appendMessage("I'm processing your request. How else can I assist you with your travel plans?", "bot");
                }, 1000);
            }
...
1. **Intent**: Identify the user's intent by looking for phrases like:
   - *"book a flight"*, *"fetch flight details"*, *"update my flight"*, *"cancel my flight"*, *"book a room"*, *"book a hotel"*, *"update hotel"*.
   - Extract one of the following intents: `book_flight`, `update_flight`, `fetch_flight`, `cancel_flight`, `book_hotel`, `update_hotel`.

2. **Boarding City**: Extract the boarding city from phrases such as:
   - *"from <city>"*, *"departing from <city>"*, *"leaving from <city>"*.

3. **Destination City**: Extract the destination city from phrases like:
   - *"to <city>"*, *"destination is <city>"*, *"destination city is <city>"*.

4. **Date of Flight**: Extract the flight date from:
   - Formats like *"on <YYYY-MM-DD>"*, *"on <DD-MM-YYYY>"*, *"for <MM/DD/YYYY>"*.
   - Relative terms such as *"tomorrow"*, *"next week"*, *"in two days"*,*"tomm"*.

5. **Name of the Passenger**: Extract the passenger’s name from:
   - *"I am <name>"*, *"My name is <name>"*, *"I would like to book under the name <name>"*.

6. **Contact Number**: Extract the phone number from:
   - *"my number is <phone_number>"*, *"contact number is <phone_number>"*, *"you can reach me at <phone_number>"*.

7. **Flight ID**: Extract the flight ID from:
   - *"the flight ID is <flight_id>"*, *"my flight ID is <flight_id>"*, *"ID is <flight_id>"*.

8. **Update Request**: Identify update requests from:
   - *"change flight date"*, *"update travel date"*, *"change destination city"*, *"change date"*, *"update boarding city"*.

9. **Update Travel Date**: Extract the new travel date from:
   - *"change my date to <YYYY-MM-DD>"*, *"reschedule to <DD-MM-YYYY>"*, *"move my flight to <MM/DD/YYYY>"*.

10. **Update Destination City**: Extract the new destination city from:
    - *"change my destination city to <city>"*, *"update my destination city to <city>"*, *"I want to fly to <city>"*.

11. **Update Boarding City**: Extract the new boarding city from:
    - *"change my boarding city to <city>"*, *"update my departure city to <city>"*, *"I want to leave from <city>"*.

12. **Email**: Extract email addresses from:
    - *"<email@example.com>"*.
...
14. **Check-in Date**: Extract the check-in date from:
    - Formats like *"check-in on <YYYY-MM-DD>"*, *"arrival date is <YYYY-MM-DD>"*.
    - Relative terms such as *"tomorrow"*, *"next week"*, *"in two days"*.

15. **Check-out Date**: Extract the check-out date from:
    - *"check-out on <YYYY-MM-DD>"*, *"departure date is <YYYY-MM-DD>"*, *"I will leave on <YYYY-MM-DD>"*.

16. **Number of Adults**: Extract the number of adults from:
    - *"for <number> adults"*, *"number of guests <number>"*, *"booking for <number> people"*.

17. **Update Choice**: Identify requests to update hotel details from:
    - *"change city"*, *"update check-in date"*, *"update check-out date"*, *"update adults"*.

18. **New City**: Extract the new city for hotel bookings from:
    - *"change my hotel city to <city>"*, *"change my city to <city>"*.

19. **New Check-in Date**: Extract the new check-in date from:
    - *"change my check-in date to <YYYY-MM-DD>"*, *"update my check-in date to <YYYY-MM-DD>"*.

20. **New Check-out Date**: Extract the new check-out date from:
    - *"change my checkout date to <YYYY-MM-DD>"*.

21. **Booking ID**: Extract the hotel booking ID from:
    - *"the booking ID is <booking_id>"*, *"my booking ID is <booking_id>"*, *"ID is <booking_id>"*.
...
def validate_input_date(date_str):
    """
    Validates and processes a date string. Handles standard date formats (YYYY-MM-DD)
    and relative terms like 'tomm' or 'tomorrow', returning a validated date.
    """
    # Handle relative terms like 'tomm' or 'tomorrow'
    if date_str.lower() in ["tomm", "tomorrow"]:
        return (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%d")  # Return tomorrow's date in YYYY-MM-DD

    # Regex pattern for YYYY-MM-DD format
    pattern = r'^\d{4}-\d{2}-\d{2}$'

    # Validate and return standard date format
    ...
    # If the input doesn't match, return None
    return None
...
def validate_contact(contact):
    """
    Validates the phone number format and ensures it is in E.164 format.
    Adds the default country code if missing.
    """
    # Default country code for India
    country_code = "+91"

    # Check if the contact starts with "+" (already in E.164 format)
    if not contact.startswith("+"):
        # Add country code if the number is local
        contact = country_code + contact

    # Check if the contact is a valid number with 10-15 digits
    if contact.startswith("+") and contact[1:].isdigit() and 10 <= len(contact[1:]) <= 15:
        return contact  # Valid number in E.164 format
