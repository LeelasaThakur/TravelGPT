...
# Initialize the Amadeus API — wrapped so a network failure doesn't crash Flask startup
try:
    amadeus = AmadeusAPI()  # Create an instance of AmadeusAPI
except Exception as e:
    print(f"⚠️  Amadeus API unavailable on startup: {e}")
    amadeus = None

def get_amadeus():
    """
    Returns the Amadeus instance, attempting to reconnect if startup failed.
    A network blip at boot won't permanently disable flight/hotel search.
    """
    global amadeus
    if amadeus is None:
        try:
            print("Attempting to reconnect to Amadeus API...")
            amadeus = AmadeusAPI()
            print("✅ Amadeus reconnected successfully.")
        except Exception as e:
            print(f"Amadeus reconnect failed: {e}")
    return amadeus
...
def send_message():
    ...
    # If not found in user_details, try to extract from the current message (avoid double LLM call if already extracted)
    if not destination_city and not active_task:
    ...
    # Only generate itinerary once per destination to avoid excessive LLM calls
    if destination_city:
        cached_dest = session.get("itinerary_city")
        if cached_dest != destination_city.lower():
            itinerary = generate_itinerary(destination_city)
            session["itinerary_city"] = destination_city.lower()
...
def generate_response(prompt, retries=3):
    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": "You are a highly intelligent and efficient flight booking assistant. Your role is to help users with tasks such as booking flights, updating travel details, retrieving flight information, and canceling bookings. Provide clear, concise, and accurate responses while guiding the user through the process step-by-step."},
    ...
    for attempt in range(retries):
        try:
            response = requests.post(SAMBANOVA_BASE_URL, headers=headers, json=payload, timeout=30)
            if response.status_code == 429:
                wait = 2 ** attempt  # 1s, 2s, 4s
                print(f"SambaNova rate limited (429). Waiting {wait}s before retry {attempt + 1}/{retries}...")
                time.sleep(wait)
                continue
            ...
            return None
    print("SambaNova API failed after all retries.")
    return None
...
21. Booking ID: Extract the hotel booking ID from:*"the booking ID is <booking_id>"*, *"my booking ID is <booking_id>"*, *"ID is <booking_id>"*.
    ...
    try:
        # Guard: if LLM call failed and returned None, return safe defaults
        if not response:
            print("Warning: generate_response returned None. Returning empty passenger_details.")
    ...
    required_fields = ["boarding_city", "destination_city", "travel_date", "name", "contact","flight_id", "update_request", "new_travel_date", "new_boarding_city", "new_destination_city", "city_name", "check_in_date", "check_out_date", "adults", "update_choice","booking_id","new_city","new_check_in_date","new_check_out_date"]  # List of required fields
...
def get_iata_code(city_name):
    try:
        if not amadeus:
            print("Amadeus API not available.")
            return None
...
def book_flight(name, contact,boarding_city, destination_city, travel_date):
    ...
    _api = get_amadeus()
    if not _api:
        return "Flight search unavailable. Amadeus service could not be reached."
    available_flights = _api.search_flights(boarding_city_code, destination_city_code, travel_date)
    if not available_flights:
        return f"No flights available for {travel_date}. Please enter a new date (YYYY-MM-DD): "
...
def display_flight(available_flights, boarding_city_code, destination_city_code):
    ...
    flight_lines = []  # Local list for formatted flight strings
    for idx, flight in enumerate(available_flights, 1):  # Iterate through available flights
        ...
        try:
            airline_name = (get_amadeus().get_airline_name(carrier_code) if get_amadeus() else carrier_code)
        except Exception:
            airline_name = carrier_code  # Fall back to raw carrier code on network error
        ...
        flight_lines.append(f"{idx}. Airline: {airline_name}, Departure: {departure_time}, Arrival: {arrival_time}, Price:{price}")  # Append flight detail

        
    # Prepare and send the message with available flights
    flights_message = "\n\n".join(flight_lines)  # Join flight details into a string
...
def handle_flight_selection(flight_choice, available_flights, name, contact, boarding_city_code, destination_city_code, travel_date):
    """
...
def search_flights_after_update(updated_details):
    ...
    try:
        ...
        if not _api2:
            return None
        try:
            available_flights = _api2.search_flights(boarding_city_code, destination_city_code, travel_date)
        except Exception as e:
...
def finalize_flight_selection(updated_details, flight_choice):
    ...
    updated_details["airline_name"] = airline_name
...
def handle_hotel_search(city_name, check_in_date, check_out_date, adults=1):
    ...
    if not api:
        ...
        return []
    try:
        city_data = api.get_city_code(city_name)
    except Exception as e:
        print(f"Amadeus get_city_code failed: {e}")
        return []
    if not city_data or not city_data.get("iataCode"):
        return []

    city_code = city_data["iataCode"]
    hotel_list = api.fetch_hotel_list_by_city(city_code)
    if not hotel_list:
        return []

    hotel_ids = [hotel["hotelId"] for hotel in hotel_list]
    offers = api.fetch_hotel_offers(hotel_ids, check_in_date, check_out_date, adults)
    if not offers:
        return []
...
def chatbot_response(user_message):
    ...
    try:
        ...
        if not active_task:  # If no active task
            ...
            # If no intent was detected, treat it as casual conversation
        if not active_task:
            return generate_response(user_message) or "Hello! How can I help you with your travel plans today?"
        ...
        traceback.print_exc()
        return "Oops! Something went wrong. Please try again or start a new conversation."
...
def handle_book_hotel(user_message):
    ...
    if not contact:
        return prompt_user("Invalid contact number. Please enter again:", "contact")
    user_details["contact"] = contact

    check_in_date = validate_input_date(user_details["check_in_date"])
    if not check_in_date:
        return prompt_user("Invalid check-in date. Please use YYYY-MM-DD:", "check_in_date")
    user_details["check_in_date"] = check_in_date

    check_out_date = validate_input_date(user_details["check_out_date"])
    if not check_out_date:
        return prompt_user("Invalid check-out date. Please use YYYY-MM-DD:", "check_out_date")
    user_details["check_out_date"] = check_out_date
...
def handle_book_flight(user_message):
    ...
    if expected_key == "flight_choice" or not user_details.get("flight_choice"):  # Check if flight choice is expected
        ...
        _api = get_amadeus()
        if not _api:
            return "Flight search is currently unavailable. The Amadeus service could not be reached."
        try:
            available_flights = _api.search_flights(boarding_city_code, destination_city_code, travel_date)
        except Exception as e:
            print(f"Error searching flights: {e}")
            return "Unable to reach the flight search service right now. Please check your internet connection and try again."

        if not available_flights:  # No flights found - ask for a new date
            return prompt_user(f"No flights available for {user_details['travel_date']}. Please enter a new travel date (YYYY-MM-DD):", "travel_date")
