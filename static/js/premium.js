/* =========================================================
    ...
    (function () {
     ...
     function appendMessage(text, role) {
       ...
       const contentHtml = isUser ? escapeHtml(text) : (window.marked ? marked.parse(text) : escapeHtml(text));
       ...
       row.innerHTML = `
         ...
         <div>
           <div class="bubble ${bubbleCls} ${!isUser ? 'markdown-body' : ''}">${contentHtml}</div>
     ...
     async function sendMessage(messageOverride) {
       ...
       try {
         ...
         if (responseJson.status === "error" || !responseJson.data) {
           appendMessage(`⚠️ ${responseJson.message}`, "bot");
           return;
         }

         const data = responseJson.data;
         window.currentConversationId = responseJson.conversation_id;
   
         // Append bot reply
         appendMessage(data.reply || "I'm not sure how to respond to that.", "bot");
   
         // Update UI from response
         if (data.options && data.options.length > 0) {
            let optionsHtml = '<div class="options-container" style="margin-top:12px;display:flex;flex-direction:column;gap:8px;">';
            data.options.forEach((opt, idx) => {
                optionsHtml += `
                    <div class="option-card" style="background:var(--bg-secondary); border:1px solid var(--border-color); padding:12px; border-radius:8px; cursor:pointer;" onclick="sendMessage('I select option ${idx + 1}')">
                        <strong>${opt.airline || opt.hotel_name || opt.name || 'Option ' + (idx + 1)}</strong><br/>
                        <span style="font-size:12px;color:var(--text-secondary)">${opt.departure_time || opt.address || ''}</span>
                        <div style="color:var(--accent-gold);margin-top:4px;">$${opt.price || ''} ${opt.currency || ''}</div>
                    </div>
                `;
            });
            optionsHtml += '</div>';
            
            // Append options directly to the viewport as a raw HTML block
            const row = document.createElement("div");
            row.className = `msg-row bot`;
            row.innerHTML = `<div style="margin-left:50px;">${optionsHtml}</div>`;
            viewport.appendChild(row);
            scrollToBottom();
         }

         const dest = extractDestinationFromMsg(text) || extractDestinationFromMsg(data.reply || "");
         updateDestinationUI(dest, null);
   
         // Count bookings
         const bookingType = detectBookingType(data.reply || "");
     ...
     function clearConversation() {
       ...
       // Notify backend to reset session by just clearing the local ID
